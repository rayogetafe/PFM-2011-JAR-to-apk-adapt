...
def main():
    ...
    lines=["type\tleague\tclubIndex\tclub\tplayerIndex\tplayer\tposition\tage\toverall\tnationality"]
    with zipfile.ZipFile(a.jar) as z:
        for resource,league in RESOURCE_TO_LEAGUE.items():
            ...
            for ti,t in enumerate(teams):
                lines.append("\t".join(("T",code,str(ti),safe(t["name"]),"-1","","-1","0","0","")))
                for pid in t["roster"]:
                    x=players[pid];overall=sum(x["attrs"][2:5])//3
                    lines.append("\t".join(("P",code,str(ti),safe(t["name"]),str(pid),safe(x["name"]),str(x["attrs"][1]),str(x["attrs"][0]),str(overall),safe(nat[(league,t["name"],x["name"])]))))
