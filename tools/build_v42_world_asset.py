...
def main():
    ...
    lines=["type\tleague\tclubIndex\tclub\tplayerIndex\tplayer\tposition\tage\toverall\tnationality\tspeed\tresistance\tquality\tmorale\tstyle\tformation"]
    with zipfile.ZipFile(a.jar) as z:
        for resource,league in RESOURCE_TO_LEAGUE.items():
            ...
            for ti,t in enumerate(teams):
                ...
                lines.append("\t".join(("T",code,str(ti),safe(t["name"]),"-1","","-1","0","0","","0","0","0","0",str(t["fixed"][14]),str(t["fixed"][15]))))
                for pid in t["roster"]:
                    x=players[pid];overall=sum(x["attrs"][2:5])//3
                    lines.append("\t".join(("P",code,str(ti),safe(t["name"]),str(pid),safe(x["name"]),str(x["attrs"][1]),str(x["attrs"][0]),str(overall),safe(nat[(league,t["name"],x["name"])]),str(x["attrs"][2]),str(x["attrs"][3]),str(x["attrs"][4]),str(x["attrs"][7]),"0","0")))
