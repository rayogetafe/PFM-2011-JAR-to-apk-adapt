...
# Show detector failure after both exact-hit and nearest-centre target attempts.
old='''            if(target<0) return false;\n\n            // Build a local menu group'''
new='''            if(target<0){\n                StringBuilder rr=new StringBuilder();\n                for(int q=0;q<count && q<8;q++) rr.append(q).append(':').append(ys[q]).append('-').append(ye[q]).append("s").append(score[q]).append(' ');\n                setTouchDebug("NO TARGET tap="+tapX+","+tapY+" rows="+count,rr.toString(),"");\n                return false;\n            }\n\n            // Build a local menu group'''
if old not in s:
