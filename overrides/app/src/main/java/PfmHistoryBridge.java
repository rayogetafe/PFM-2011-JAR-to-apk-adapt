...
public final class PfmHistoryBridge {
    ...
    private static int teamId(String club){try{dw[] all=(dw[])field(cp.class,"a",dw[].class).get(null);if(all!=null)for(int i=0;i<all.length;i++){dw t=all[i];if(t!=null){String n="";try{n=pfmAccess50.name(t);}catch(Throwable ignored){}if(norm(n).equals(norm(club)))return i;}}}catch(Throwable ignored){}return -1;}
