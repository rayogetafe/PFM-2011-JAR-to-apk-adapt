...
public final class NativeLeagueCenter {
    ...
    private static String playerBlock(Activity a,int teamIndex){StringBuilder b=new StringBuilder();for(String s:rows("playerRows",new Class<?>[]{Integer.TYPE},new Object[]{Integer.valueOf(teamIndex)})){Player p=Player.parse(s);b.append('#').append(p.number).append(' ').append(p.name).append("  ").append(p.pos).append('\n').append("ST ").append(p.st).append(" AP ").append(p.ap).append("  G ").append(p.g).append(" A ").append(p.a).append("  RAT ").append(p.rating()).append("\n\n");}return b.toString();}
    /** Roadmap step: every league result now opens a two-sided match card. */
    private static void showMatch(Activity a,Match m){
        ...
        LinearLayout sides=new LinearLayout(a);sides.setOrientation(LinearLayout.HORIZONTAL);
        String[] names={m.home,m.away};int[] indexes={m.homeIndex,m.awayIndex};for(int i=0;i<2;i++){LinearLayout side=new LinearLayout(a);side.setOrientation(LinearLayout.VERTICAL);side.setPadding(dp(a,5),0,dp(a,5),0);TextView h=text(a,names[i]+"\nSEASON PLAYER STATS",13f,true);h.setGravity(android.view.Gravity.CENTER);side.addView(h);TextView ps=text(a,playerBlock(a,indexes[i]),11f,false);ps.setTypeface(Typeface.MONOSPACE);side.addView(ps);sides.addView(side,new LinearLayout.LayoutParams(0,-2,1));}body.addView(sides);
        String report="";try{report=String.valueOf(bridge().getMethod("eventReport",Integer.TYPE).invoke(null,Integer.valueOf(m.round)));}catch(Throwable ignored){}if(m.done&&m.user&&report.length()>0){TextView events=text(a,"USER MATCH EVENTS\n"+report,12f,false);events.setPadding(dp(a,4),dp(a,8),dp(a,4),dp(a,8));body.addView(events);}
