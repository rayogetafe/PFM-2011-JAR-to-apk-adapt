...
public final class NativeSquad {
    ...
    private static void showPlayer(Activity a,PlayerRow p){
        ...
        b.append("Condition\n");
        b.append("Fatigue: ").append(p.fatigue).append(" / 100 (lower is better)\n");
        b.append("Status: ").append(p.status()).append("\n");
        b.append("Cards: ").append(p.yellow).append(" Y / ").append(p.red).append(" R\n\n");
        b.append("Season\n");
        b.append("Starts: ").append(p.starts).append("\n");
        b.append("Appearances: ").append(p.apps).append("\n");
        b.append("Goals: ").append(p.goals).append("\n");
    ...
    private static boolean matchesFilter(PlayerRow p,String filter){
        ...
        if("Unavailable".equals(filter))return p.unavailable();
        return filter.equals(p.pos);
    }

    private static Comparator<PlayerRow> comparator(String sort){
        if("Position".equals(sort))return (a,b) -> {
            int c=a.pos.compareTo(b.pos); if(c!=0)return c; return a.number-b.number;
        };
        if("Starts".equals(sort))return (a,b) -> b.starts-a.starts;
        if("Goals".equals(sort))return (a,b) -> b.goals-a.goals;
        if("Rating".equals(sort))return (a,b) -> {
            if(a.ratingMatches==0&&b.ratingMatches>0)return 1;
            if(b.ratingMatches==0&&a.ratingMatches>0)return -1;
            return b.rating10-a.rating10;
        };
    ...
    public static void show(Activity a){
        ...
        controls.setPadding(0,0,0,dp(a,5));

        Spinner filter=new Spinner(a);
        String[] filters={"All","GK","DEF","MID","FW","Unavailable"};
        ArrayAdapter<String> fa=new ArrayAdapter<>(a,android.R.layout.simple_spinner_item,filters);
        fa.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        filter.setAdapter(fa);
        controls.addView(filter,new LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f));

        Spinner sort=new Spinner(a);
        String[] sorts={"Squad order","Position","Starts","Goals","Rating","Fatigue"};
        ArrayAdapter<String> sa=new ArrayAdapter<>(a,android.R.layout.simple_spinner_item,sorts);
        ...
        final ArrayList<PlayerRow> visible=new ArrayList<>();
        ArrayAdapter<PlayerRow> adapter=new ArrayAdapter<PlayerRow>(a,android.R.layout.simple_list_item_1,visible){
            @Override public View getView(int position,View convertView,ViewGroup parent){
                ...
                if(convertView instanceof LinearLayout && ((LinearLayout)convertView).getChildCount()==3){
                    ...
                    l3=(TextView)row.getChildAt(2);
                }else{
                    row=new LinearLayout(a);
                    row.setOrientation(LinearLayout.VERTICAL);
                    row.setPadding(dp(a,12),dp(a,7),dp(a,12),dp(a,7));
                    row.setMinimumHeight(dp(a,72));
                    l1=text(a,"",17f,true);
                    l2=text(a,"",13f,false);
                    l3=text(a,"",12f,false);
                    row.addView(l1);
                ...
                l3.setText("FAT "+p.fatigue+"   "+p.status()+"   YC "+p.yellow+" RC "+p.red);
                return row;
            }
        };
        list.setAdapter(adapter);
        list.setOnItemClickListener((parent,view,position,id) -> showPlayer(a,visible.get(position)));
        ...
        Runnable refresh=() -> {
            ...
            visible.clear();
            for(PlayerRow p:source)if(matchesFilter(p,f))visible.add(p);
            Comparator<PlayerRow> cmp=comparator(s);
            if(cmp!=null)Collections.sort(visible,cmp);
            adapter.notifyDataSetChanged();
        };
        filter.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){
            public void onItemSelected(android.widget.AdapterView<?> p,View v,int pos,long id){refresh.run();}
        ...
        dlg.setOnShowListener(x -> {
            ...
            if(w!=null)w.setLayout(WindowManager.LayoutParams.MATCH_PARENT,
                    Math.round(a.getResources().getDisplayMetrics().heightPixels*0.90f));
        });
        dlg.show();
    }
}
