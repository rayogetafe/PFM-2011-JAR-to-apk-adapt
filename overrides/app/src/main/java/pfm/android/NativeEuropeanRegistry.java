...
public final class NativeEuropeanRegistry {
 ...
 private static void showRoster(Activity a,Club club){
  ...
  note.setText(names.size()+" supplied names · "+conflicts+" flagged here. This is the original research list; use LIVE SQUAD for imported career players.");root.addView(note);
 ...
 private static void showLiveRoster(Activity a,Club club){
  ArrayList<String> names=new ArrayList<String>();int estimated=0;
  try{
   NativeWorldCenter.load(a);NativeCareerStore.Career career=NativeCareerStore.ensure(a);String owner=null;
   for(NativeCareerStore.Club x:career.clubs)if(x.league.startsWith("eu_")&&x.name.equals(club.name)){owner=x.id;break;}
   if(owner==null)throw new IllegalStateException("Club absent from native career");
   for(NativeCareerStore.Player p:career.players)if(owner.equals(p.owner)){
    if("ESTIMATED".equals(p.provenance))estimated++;
    names.add(p.name+"  ·  "+NativeGlobalMarket.pos(p.position)+"  OVR "+p.overall+"  age "+p.age+"\n"+p.provenance);
   }
  }catch(Throwable error){new AlertDialog.Builder(a).setMessage("Native squad unavailable: "+error.getMessage()).setPositiveButton("Close",null).show();return;}
  LinearLayout root=new LinearLayout(a);root.setOrientation(LinearLayout.VERTICAL);
  TextView note=new TextView(a);note.setPadding(dp(a,12),dp(a,8),dp(a,12),dp(a,8));
  note.setText(names.size()+" career players · "+estimated+" estimated ratings. All ages and finances are gameplay estimates. Transfers update this list.");root.addView(note);
  ListView list=new ListView(a);list.setAdapter(new ArrayAdapter<String>(a,android.R.layout.simple_list_item_1,names));root.addView(list,new LinearLayout.LayoutParams(-1,0,1));
  AlertDialog dialog=new AlertDialog.Builder(a).setTitle(club.name+" · live squad").setView(root).setPositiveButton("Close",null).create();
  dialog.setOnShowListener(v->{Window window=dialog.getWindow();if(window!=null)window.setLayout(-1,Math.round(a.getResources().getDisplayMetrics().heightPixels*.85f));});dialog.show();
 }
 ...
 public static void show(Activity a){final boolean pause=NativePause.begin();ArrayList<Club> source=all(a),shown=new ArrayList<Club>();LinearLayout root=new LinearLayout(a);root.setOrientation(LinearLayout.VERTICAL);TextView status=new TextView(a);status.setPadding(dp(a,8),dp(a,6),dp(a,8),dp(a,6));status.setText("129 European clubs now have native career squads. Positions and ages are archive-checked where matched; ratings, wages and values remain gameplay estimates.");root.addView(status);Spinner country=new Spinner(a);TreeSet<String> cs=new TreeSet<String>();for(Club x:source)cs.add(x.country);ArrayList<String> labels=new ArrayList<String>();labels.add("ALL COUNTRIES");labels.addAll(cs);country.setAdapter(new ArrayAdapter<String>(a,android.R.layout.simple_spinner_dropdown_item,labels));root.addView(country);ListView list=new ListView(a);ArrayAdapter<Club> ad=adapter(a,shown);list.setAdapter(ad);root.addView(list,new LinearLayout.LayoutParams(-1,0,1));Runnable refresh=()->{shown.clear();String selected=String.valueOf(country.getSelectedItem());for(Club x:source)if("ALL COUNTRIES".equals(selected)||x.country.equals(selected))shown.add(x);status.setText("Shown "+shown.size()+" / "+source.size()+" clubs  ·  2584 native players\n2298 players season-confirmed; 286 source gaps explicitly marked. Tap a club for its live squad. Tap a club for its live squad.");ad.notifyDataSetChanged();};country.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){public void onItemSelected(android.widget.AdapterView<?> p,View v,int n,long id){refresh.run();}public void onNothingSelected(android.widget.AdapterView<?> p){}});list.setOnItemClickListener((p,v,n,id)->{Club x=shown.get(n);new AlertDialog.Builder(a).setTitle(x.name).setMessage(x.country+"\nEuropean status: "+x.europe+"\nReputation "+x.rep+" / Finance "+x.fin+" / Academy "+x.acad+"\nTransfer profile: "+x.profile+"\nPriority: "+x.priority+"\nActive from: "+x.activeFrom+(x.note.length()>0?"\n\n"+x.note:"")).setNegativeButton("Live squad",(dialog,which)->showLiveRoster(a,x)).setNeutralButton("Source draft",(dialog,which)->showRoster(a,x)).setPositiveButton("Close",null).show();});refresh.run();AlertDialog d=new AlertDialog.Builder(a).setTitle("European Club Registry").setView(root).setPositiveButton("Close",null).create();d.setOnDismissListener(v->NativePause.end(pause));d.setOnShowListener(v->{Window w=d.getWindow();if(w!=null)w.setLayout(-1,Math.round(a.getResources().getDisplayMetrics().heightPixels*.94f));});d.show();}
}
