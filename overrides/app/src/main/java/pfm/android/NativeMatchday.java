...
public final class NativeMatchday {
    ...
    private static String reportSummary(String[] lines){String match="",score="";for(String line:lines){if(line.startsWith("MATCH "))match=line.substring(6);else if(line.startsWith("SCORE "))score=line;}return match.length()>0?match+(score.length()>0?"   •   "+score:""):score;}
