...
final class NativeAiMarket {
    ...
    static synchronized int run(Context context)throws Exception{
        ...
        int limit=windowCap();if(limit<=0)return 0;
        ...
        int moved=0,firstDeal=c.deals.size();
        for(NativeCareerStore.Club buyer:buyers){
            ...
            NativeCareerStore.Player best=null;NativeCareerStore.Club seller=null;int bestScore=Integer.MIN_VALUE,bestFee=0,bestWage=0,bestYears=0;
            for(NativeCareerStore.Player p:c.players){
                ...
                int improvement=p.overall-needScore,required=p.age>=29?1:2;if(stronger<required||improvement<(p.age>=30?1:2))continue;
                int pct=(p.age<=23?110:p.age>=30?85:98)+(((p.id+":"+buyer.id+":"+seed).hashCode()&0x7fffffff)%(p.age<=23?21:16));
                int fee=NativeGlobalMarket.quoted((int)Math.max(100000L,(long)p.value*pct/100L));
                int wage=(int)Math.max(50000L,((long)p.contractWage*(100+((p.id+buyer.id+seed).hashCode()&15))/100L+5000L)/10000L*10000L),years=p.age<=23?4:p.age>=30?2:3;
                if(fee>buyer.balance||payroll(buyerSquad)+wage>buyer.wageBudget)continue;
                int ageFit=p.age<=24?180:p.age<=29?80:-(p.age-29)*35,score=improvement*1000+ageFit-fee/200000+((p.id+buyer.id+seed).hashCode()&255);
                if(score>bestScore){best=p;seller=owner;bestScore=score;bestFee=fee;bestWage=wage;bestYears=years;}
            }
            if(best==null)continue;
            buyer.balance-=bestFee;seller.balance=(int)Math.min(Integer.MAX_VALUE,(long)seller.balance+bestFee);
            String from=best.owner;squads.get(from).remove(best);buyerSquad.add(best);best.owner=buyer.id;best.contractWage=bestWage;best.contractUntil=c.season+bestYears;
            NativeCareerStore.Deal d=new NativeCareerStore.Deal();d.player=best.id;d.from=from;d.to=buyer.id;d.fee=bestFee;d.wage=bestWage;d.years=bestYears;d.season=c.season;d.ai=true;c.deals.add(d);moved++;
        }
        c.aiWindow=window;
        String audit=NativeCareerStore.commitAi(context,c);if(!"OK".equals(audit))throw new IllegalStateException(audit);
        for(int i=firstDeal;i<c.deals.size();i++)NativeCareerStore.postMail(c,c.deals.get(i));
        return moved;
    }
}
