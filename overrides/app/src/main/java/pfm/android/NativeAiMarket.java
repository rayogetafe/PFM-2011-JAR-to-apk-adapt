...
final class NativeAiMarket {
 ...
 static synchronized int run(Context context,boolean calendarTick)throws Exception{
  ...
  ArrayList<NativeCareerStore.Club> buyers=new ArrayList<NativeCareerStore.Club>();for(NativeCareerStore.Club club:c.clubs)if(club.activeFrom<=dealSeason&&(user==null||!user.id.equals(club.id))&&!club.league.equals(c.activeLeague))buyers.add(club);
  ...
  for(int pass=0;pass<3&&moved<limit;pass++)for(NativeCareerStore.Club buyer:buyers){if(moved>=limit)break;int minLeague=Integer.MAX_VALUE;for(String code:NativeWorldCenter.allCodes())if(!code.equals(c.activeLeague))minLeague=Math.min(minLeague,leagueMoves.containsKey(code)?leagueMoves.get(code):0);int buyerLeague=leagueMoves.containsKey(buyer.league)?leagueMoves.get(buyer.league):0;if(buyerLeague>minLeague+1)continue;int bought=incoming.containsKey(buyer.id)?incoming.get(buyer.id):0;if(bought>=clubLimit||bought>pass)continue;ArrayList<NativeCareerStore.Player> buyerSquad=squads.get(buyer.id);if(buyerSquad.size()>=30)continue;
   ...
   for(NativeCareerStore.Player p:c.players){if(p.position!=need||p.owner.equals(buyer.id)||recent.contains(p.id)||(user!=null&&p.owner.equals(user.id)))continue;NativeCareerStore.Club owner=clubs.get(p.owner);ArrayList<NativeCareerStore.Player> sellerSquad=squads.get(p.owner);if(owner==null||owner.activeFrom>dealSeason||owner.league.equals(c.activeLeague)||sellerSquad.size()<=(owner.league.startsWith("eu_")?13:18))continue;
    int sold=outgoing.containsKey(owner.id)?outgoing.get(owner.id):0;if(sold>=clubLimit)continue;int min=owner.league.startsWith("eu_")?(need==0?1:need==1?3:need==2?3:2):(need==0?2:need==1?5:need==2?5:4);if(count(sellerSquad,need)<=min)continue;int stronger=0;for(NativeCareerStore.Player q:sellerSquad)if(q.position==need&&q.overall>p.overall)stronger++;
