...
/** Per-save-slot goalkeeper match ledger. Exact CS/GA; saves are a deterministic added simulation stat. */
public final class pfmGoalkeeperStats31 {
    ...
    private static int modelledSaves(int playerId,ci keeper,int round,int conceded){int h=playerId*1103515245+round*12345+pfm2.getSeason()*97;h^=(h>>>16);int saves=1+Math.abs(h%5);if(quality(keeper)>=78)saves++;if(conceded>=3&&saves>1)saves--;return Math.max(0,saves);}
