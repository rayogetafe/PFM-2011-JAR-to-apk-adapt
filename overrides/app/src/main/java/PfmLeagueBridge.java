...
public final class PfmLeagueBridge {
    ...
    public static String rosterAudit(){StringBuilder out=new StringBuilder();try{dw[] ts=teams();ci[] ps=players();boolean[] seen=new boolean[ps.length];int playerCount=0,duplicates=0,orderIssues=0,depthAlerts=0,small=0;ArrayList<String> full=new ArrayList<String>(),near=new ArrayList<String>();int rounds=played();for(int ti=0;ti<ts.length;ti++){dw t=ts[ti];if(t==null)continue;short[] ids=order(t);int n=count(t),last=-1;int[] d=new int[4];if(n<18)small++;for(int i=0;i<n&&ids!=null&&i<ids.length;i++){int id=ids[i]&65535;if(id>=ps.length||ps[id]==null)continue;playerCount++;if(seen[id])duplicates++;seen[id]=true;int po=ub(ps[id],"b");if(po>=0&&po<4)d[po]++;if(po<last)orderIssues++;last=po;int st=pfmPlayerStats.starts(id);String label=teamName(t)+" — "+playerName(ps[id])+" ("+pos(po)+", "+st+" ST)";if(rounds>=37&&st>=rounds-1)full.add(label);else if(rounds>=20&&st>=rounds-2)near.add(label);}if(d[0]<2||d[1]<5||d[2]<5||d[3]<3)depthAlerts++;}out.append("ROSTER AUDIT\nTeams ").append(ts.length).append("   Players ").append(playerCount).append("\nDuplicate ownership ").append(duplicates).append("   Position-order issues ").append(orderIssues).append("\nSquads under 18 ").append(small).append("   Depth alerts ").append(depthAlerts).append("\n\n37–38 START CALIBRATION\nCurrent ").append(full.size()).append("   Target ").append(targetTotal()).append("\nRotation tuning: fatigue threshold 23, penalty 7 (moderate midpoint).\n");if(!full.isEmpty()){out.append("\nFULL-SEASON / ONE MISSED\n");for(String s:full)out.append(s).append('\n');}if(!near.isEmpty()){out.append("\nNEAR FULL-SEASON\n");for(String s:near)out.append(s).append('\n');}}catch(Throwable ignored){}return out.toString().trim();}
