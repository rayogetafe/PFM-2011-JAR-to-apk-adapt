...
public final class PfmLeagueBridge {
    ...
    private static String leagueName(){try{int x=field(ea.class,"a",Integer.TYPE).getInt(null);String[] n={"England","Spain","Italy","Germany","France","International"};return x>=0&&x<n.length?n[x]:"";}catch(Throwable t){return "";}}
    ...
    public static String[] playerRows(int teamIndex){ArrayList<String> out=new ArrayList<String>();try{dw[] ts=teams();ci[] ps=players();if(teamIndex<0||teamIndex>=ts.length)return new String[0];dw t=ts[teamIndex];short[] ids=order(t);int n=count(t);for(int i=0;i<n&&ids!=null&&i<ids.length;i++){int id=ids[i]&65535;if(id>=ps.length||ps[id]==null)continue;ci p=ps[id];int rat=pfmPlayerStats.averageRating10(id),rm=pfmPlayerStats.ratingMatches(id),outRounds=Math.max(statByte(pfmCondition60.class,"injury",id),pfmDiscipline70.roundsOut(id));String pn=playerName(p);out.add(id+"\t"+ub(p,"f")+"\t"+pn+"\t"+pos(ub(p,"b"))+"\t"+ub(p,"a")+"\t"+((ub(p,"c")+ub(p,"d")+ub(p,"e"))/3)+"\t"+pfmPlayerStats.starts(id)+"\t"+pfmPlayerStats.appearances(id)+"\t"+ub(p,"g")+"\t"+pfmContrib80.assists(id)+"\t"+rat+"\t"+rm+"\t"+pfmGoalkeeperStats31.starts(id)+"\t"+pfmGoalkeeperStats31.cleanSheets(id)+"\t"+pfmGoalkeeperStats31.conceded(id)+"\t"+pfmGoalkeeperStats31.saves(id)+"\t"+statByte(pfmDiscipline70.class,"yellow",id)+"\t"+statByte(pfmDiscipline70.class,"red",id)+"\t"+statByte(pfmCondition60.class,"fatigue",id)+"\t"+outRounds+"\t"+pfmNationality42.get(leagueName(),teamName(t),pn));}}catch(Throwable ignored){}return out.toArray(new String[out.size()]);}
    ...
    /** TSV: teamIndex, teamName, followed by the normal player row. */
    public static String[] leaguePlayerRows(){ArrayList<String> out=new ArrayList<String>();try{for(String t:teamRows()){String[] q=t.split("\t",-1);int ti=Integer.parseInt(q[0]);for(String p:playerRows(ti))out.add(ti+"\t"+q[2]+"\t"+p);}}catch(Throwable ignored){}return out.toArray(new String[out.size()]);}

    /** End-to-end roster ownership/depth/order audit plus full-season starter detail. */
    public static String rosterAudit(){StringBuilder out=new StringBuilder();try{dw[] ts=teams();ci[] ps=players();boolean[] seen=new boolean[ps.length];int playerCount=0,duplicates=0,orderIssues=0,depthAlerts=0,small=0;ArrayList<String> full=new ArrayList<String>(),near=new ArrayList<String>();int rounds=played();for(int ti=0;ti<ts.length;ti++){dw t=ts[ti];if(t==null)continue;short[] ids=order(t);int n=count(t),last=-1;int[] d=new int[4];if(n<18)small++;for(int i=0;i<n&&ids!=null&&i<ids.length;i++){int id=ids[i]&65535;if(id>=ps.length||ps[id]==null)continue;playerCount++;if(seen[id])duplicates++;seen[id]=true;int po=ub(ps[id],"b");if(po>=0&&po<4)d[po]++;if(po<last)orderIssues++;last=po;int st=pfmPlayerStats.starts(id);String label=teamName(t)+" — "+playerName(ps[id])+" ("+pos(po)+", "+st+" ST)";if(rounds>=37&&st>=rounds-1)full.add(label);else if(rounds>=20&&st>=rounds-2)near.add(label);}if(d[0]<2||d[1]<5||d[2]<5||d[3]<4)depthAlerts++;}out.append("ROSTER AUDIT\nTeams ").append(ts.length).append("   Players ").append(playerCount).append("\nDuplicate ownership ").append(duplicates).append("   Position-order issues ").append(orderIssues).append("\nSquads under 18 ").append(small).append("   Depth alerts ").append(depthAlerts).append(" (target 2/5/5/4)\n\n37–38 START CALIBRATION\nCurrent ").append(full.size()).append("   Target ").append(targetTotal()).append("\nRotation tuning: fatigue threshold 24, penalty 6 (v37 calibration).\n");if(!full.isEmpty()){out.append("\nFULL-SEASON / ONE MISSED\n");for(String s:full)out.append(s).append('\n');}if(!near.isEmpty()){out.append("\nNEAR FULL-SEASON\n");for(String s:near)out.append(s).append('\n');}}catch(Throwable ignored){}return out.toString().trim();}
