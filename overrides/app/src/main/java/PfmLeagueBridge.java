...
public final class PfmLeagueBridge {
    ...
    /** TSV: id, number, name, pos, age, OVR, ST, AP, G, A, RAT10, ratingMatches, GKst, CS, GA, SV. */
    public static String[] playerRows(int teamIndex){ArrayList<String> out=new ArrayList<String>();try{dw[] ts=teams();ci[] ps=players();if(teamIndex<0||teamIndex>=ts.length)return new String[0];dw t=ts[teamIndex];short[] ids=order(t);int n=count(t);for(int i=0;i<n&&ids!=null&&i<ids.length;i++){int id=ids[i]&65535;if(id>=ps.length||ps[id]==null)continue;ci p=ps[id];int rat=pfmPlayerStats.averageRating10(id),rm=pfmPlayerStats.ratingMatches(id);out.add(id+"\t"+ub(p,"f")+"\t"+playerName(p)+"\t"+pos(ub(p,"b"))+"\t"+ub(p,"a")+"\t"+((ub(p,"c")+ub(p,"d")+ub(p,"e"))/3)+"\t"+pfmPlayerStats.starts(id)+"\t"+pfmPlayerStats.appearances(id)+"\t"+ub(p,"g")+"\t"+pfmContrib80.assists(id)+"\t"+rat+"\t"+rm+"\t"+pfmGoalkeeperStats31.starts(id)+"\t"+pfmGoalkeeperStats31.cleanSheets(id)+"\t"+pfmGoalkeeperStats31.conceded(id)+"\t"+pfmGoalkeeperStats31.saves(id));}}catch(Throwable ignored){}return out.toArray(new String[out.size()]);}
